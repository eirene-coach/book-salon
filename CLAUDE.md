...
아이를 키우면서 현실에서 의미를 찾지 못하는 엄마들(일을 쉬거나, 계속하거나, 새로운 일을 찾는)을 위한 21일 비대면 북코칭 프로그램. 벤저민 하디의 『퓨처셀프』를 함께 읽으며 미래의 나를 설계하고 지금 삶의 의미와 활력을 찾는다. 하루 3~5분 코치 영상 + 해당 페이지 읽기 + 코칭 질문 응답 구조. 주5회 기준 4주(약 한 달). 코치가 직접 피드백을 작성하는 비동기 쌍방향 구조.
...
Cohort → DailyContent (1:N, day_number 1~21, 오리엔테이션 포함 총 21개 영상)
...
### 프로그램 소개 섹션
- **섹션 제목**: 21일 여정, 하루 영상 1편, 코치 1명 (주5회 기준 4주)
- **가치1 — 짧은 시간**
  - 제목: 하루 3~5분이면 충분합니다
  - 설명: 코치의 친절한 가이드 영상 1편 + 해당 페이지 읽기. 어린이집 등원 후 커피 한 잔과 함께. 긴 시간이 없어도 생각은 깊어집니다.
- **가치2 — 코치와의 연결**
  - 제목: 혼자 읽지 않아도 됩니다
  - 설명: 매일 코치가 질문을 보내고, 당신의 응답에 직접 피드백을 씁니다. 비동기지만 단절되지 않는 연결.
- **가치3 — 자기 발견**
  - 제목: 자극으로 끝나지 않는 독서
  - 설명: '좋은 책이었다'로 끝나지 않도록. 21개의 코칭 질문이 책의 언어를 내 삶의 언어로 바꿔줍니다.
...
- **후기2** — 김수현 / 경력단절 5년, 복직 준비 중
  > 4주가 끝날 때쯤 '다시 일하고 싶다'에서 '어떤 일을 하고 싶다'로 바뀌었어요. 질문이 구체적이어서 막연하게 좋은 게 아니라 내 이야기로 연결됐어요. 21개 응답을 다시 읽는데 그게 내 자서전 같았어요.
